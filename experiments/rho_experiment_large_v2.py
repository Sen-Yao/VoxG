...
def sampled_contrastive_loss(Z1, Z2, indices, temperature=0.2, num_neg=1024):
    """
    采样对比损失
    ...
    # 正样本：对角线
    pos = torch.exp(torch.sum(z1 * z2, dim=1) / temperature)  # [N]
    
    # 负样本：随机采样
    neg_indices = torch.randperm(Z2.shape[0])[:num_neg].to(Z2.device)
    z2_neg = Z2[neg_indices]  # [num_neg, D]
    z2_neg = F.normalize(z2_neg, dim=1)
    
    # z1 @ z2_neg.T
    neg_sim = torch.matmul(z1, z2_neg.T) / temperature  # [N, num_neg]
    neg = torch.exp(neg_sim).sum(dim=1)  # [N]
...
def main():
    ...
    parser.add_argument('--num_neg', type=int, default=1024)
    ...
    normal_indices = torch.tensor(normal_for_train_idx, device=device)
    ...
    for epoch in range(args.epochs):
        ...
        # 分批计算对比损失
        total_loss = 0.0
        num_batches = (len(normal_for_train_idx) + args.batch_size - 1) // args.batch_size
        
        for b in range(num_batches):
            start = b * args.batch_size
            end = min((b + 1) * args.batch_size, len(normal_for_train_idx))
            batch_indices = normal_indices[start:end]
            
            loss = sampled_contrastive_loss(
                Z_cw, Z_cc, batch_indices,
                temperature=0.2,
                num_neg=args.num_neg
            )
            
            optimizer.zero_grad()
            loss.backward(retain_graph=(b < num_batches - 1))
            optimizer.step()
            
            total_loss += loss.item() * (end - start)
        
        avg_loss = total_loss / len(normal_for_train_idx)
        ...
        if (epoch + 1) % 50 == 0 or epoch == 0:
            ...
            print(f"Epoch {epoch+1:3d}: loss={avg_loss:.4f}, CW={auc_cw:.4f}, CC={auc_cc:.4f}, Both={auc_both:.4f}")
